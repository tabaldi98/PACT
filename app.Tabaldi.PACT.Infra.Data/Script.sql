...
IF NOT EXISTS(SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME = 'Enabled' AND TABLE_NAME = 'Clients' AND TABLE_CATALOG = 'FisioPACTApp')
BEGIN
	EXEC('ALTER TABLE Clients ADD [Enabled] BIT NULL')
	EXEC('UPDATE Clients SET [Enabled] = 1')
	EXEC('ALTER TABLE Clients ALTER COLUMN [Enabled] BIT NOT NULL')
END
